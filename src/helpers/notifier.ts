...
import { isValidArray, isValidFunction, isValidObject } from './validators';
...
class ChangeNotifier {
  ...
  static notify(eventName: string, data: any, caller: string = 'notifier_default') {
    ...
    if (isValidArray(Array.from(listenerCbs)) && data !== null) {
      if (isValidObject(data)) {
        notifierState[eventName] = { ...notifierState[eventName], ...data };
      } else {
        notifierState[eventName] = data;
  ...
  static listen(eventName: string, callback: Function, state = {}) {
    if (!listeners[eventName] && isValidFunction(callback)) {
      ...
      const callbackArr: any = [...listeners[eventName]];
    ...
    return (caller: string, resetState: boolean) => {
      if (listeners[eventName]) {
        console.log(`REMOVING EVENT LISTENER FOR EVENT : ${eventName} - CALLER : ${caller}`);
        delete listeners[eventName];
        if (resetState && notifierState[eventName]) {
          console.log(`RESETTING STATE FOR EVENT : ${eventName} - CALLER : ${caller}`);
